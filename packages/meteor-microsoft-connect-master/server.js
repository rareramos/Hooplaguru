// OVERRIDE CORE METHOD for custom microsoft user selection on email
...
//
// @param serviceName {String} Service name (eg, microsoft).
...
Accounts.externalServiceSelector = function(
  ...
  //eg externalServiceSelectorMicrosoft
...
Accounts.externalServiceSelectorMicrosoft = function(
  serviceName, serviceData, options){
  var selector = {};
  // XXX Temporary special case for Microsoft. (Issue #629)
  //   The serviceData.id will be a string representation of an integer.
  //   We want it to match either a stored string or int representation.
  //   This is to cater to earlier versions of Meteor storing microsoft
  //   user IDs in number form, and recent versions storing them as strings.
  //   This can be removed once migration technology is in place, and microsoft
...
//our custom microsoft selector to also select users on microsoft-email
Accounts.externalServiceSelectorMicrosoft = function(
  serviceName, serviceData, options){
  var serviceIdKey = "services." + serviceName + ".id";
  var selector = {};
...
Meteor.methods({
  connectUserWithMicrosoft: function (token, secret) {
    //errors
    if (! this.userId)
      throw new Meteor.Error(403, "user must be loggedin");

    var user = Meteor.user();
    if (user.services && user.services.microsoft)
      throw new Meteor.Error(403, "user can not have a microsoft connected account");

    if (Meteor.isServer) {
      var tData = Microsoft.retrieveCredential(token, secret);
      console.log(tData);
      if (! tData)
        throw new Meteor.Error(403, "not able to retreive fb data");

      //check if no accounts exists for this microsoft user
      var existing = Meteor.users.find({'services.microsoft.id': tData.serviceData.id}).count();
      if(existing)
        throw new Meteor.Error(403, "user can not have a microsoft connected account");

      Meteor.users.update(this.userId, {$set: {'services.microsoft': tData.serviceData}});
    }
  }
});
